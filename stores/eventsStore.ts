    ...
    getters: {
        sortedEvents(): Event[] {
            return this.events.sort((a: Event, b: Event) => {
                const dateA: Date = new Date(a.year, a.month - 1, a.day, a.hour, a.minute);
                const dateB: Date = new Date(b.year, b.month - 1, b.day, b.hour, b.minute);
                return dateA.getTime() - dateB.getTime();
            });
        },
        upcomingEvents(): Event[] {
            const now: Date = new Date();
            return this.sortedEvents.filter((event: Event): boolean => new Date(event.year, event.month - 1, event.day, event.hour, event.minute) > now);
        },
        pastEvents(): Event[] {
            const now: Date = new Date();
            return this.sortedEvents.filter((event: Event): boolean => new Date(event.year, event.month - 1, event.day, event.hour, event.minute) < now);
